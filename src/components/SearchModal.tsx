  ...
  return (
    ...
    >
      <div className="modal__header">
        <h3 className="modal__title" id="search-dialog-title">
          검색
        </h3>
        <button
          type="button"
        ...
        <form className="search" onSubmit={onSubmit}>
          ...
          {!searchReady && (
            <p className="search-hint">
              검색 가능한 번역이 없습니다. 설정에서 언어 데이터를 내려받아
              주세요.
            </p>
          )}
        ...
        <div className="modal__content">
          ...
          <ul className="search-results__list">
            {searchResults.map((result) => (
              <li
                key={`${result.translationId}-${result.bookNumber}-${result.chapter}-${result.verse}`}
              >
                <button
                  type="button"
                ...
                >
                  <span className="result-meta">
                    ...
                    <span className="result-meta__translation">
                      {result.translation}
                    </span>
                  </span>
            ...
            {searchResults.length >= SEARCH_LIMIT && (
              <li className="hint">
                검색 결과가 많아 {SEARCH_LIMIT}개만 표시합니다.
              </li>
            )}
          </ul>
        </div>
