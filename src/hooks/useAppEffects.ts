...
type ThemeParams = {
  ...
  manualTheme: boolean
  setTheme: (value: 'light' | 'dark' | 'amoled') => void
}
...
type SelectedVerseParams = {
  ...
  setSelectedVerse: (value: number | null) => void
  focusTarget: FocusTarget
...
export const useFontScaleEffects = ({ fontScale, setFontScale, baseFontScale }: FontScaleParams) => {
...
export const useThemeEffects = ({ theme, manualTheme, setTheme }: ThemeParams) => {
  useEffect(() => {
    if (typeof window === 'undefined') {
      return
    }
...
export const useModalAccessibility = (showSearch: boolean, showSettings: boolean) => {
  useEffect(() => {
...
export const useSearchTimeoutCleanup = (ref: MutableRefObject<number | null>) => {
  useEffect(
    () => () => {
      if (ref.current !== null) {
